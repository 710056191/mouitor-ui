...
import appealNetworkApi from '@/api/report'

export default {
  name: 'NetworkReportEdit',
  ...
  methods: {
    fetch(params) {
      return appealNetworkApi.findNetworkById({ id: params.id }).then(ret => {
        return {
          ...ret.data,
        }
      })
    ...
    update(data, isNew) {
      // const updateApi = isNew ? appealNetworkApi.appealNetwork : appealNetworkApi.appealNetwork
      return appealNetworkApi
        .appealNetwork(data).then(ret => {
          this.$message({
            message: ret.message,
            type: 'success',
          })
